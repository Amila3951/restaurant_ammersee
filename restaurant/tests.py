...
class MakeReservationViewTest(TestCase):
    ...
    def test_make_reservation_valid_form(self):
        ...
        )  # Check if the form is valid, print errors if not

        response = self.client.post(
            reverse("restaurant:make_reservation"), data
        )  # Simulate a POST request

        self.assertEqual(
            Reservation.objects.count(), 1
        )  # Check if one reservation was created
        self.assertEqual(
    ...
    def test_make_reservation_invalid_form(self):
        ...
        self.assertEqual(
            Reservation.objects.count(), 0
        )  # Check that no reservations were created
        self.assertEqual(
            response.status_code, 200
        )  # Expect the same page to be rendered with errors (status code 200)
